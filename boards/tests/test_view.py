...
class BoardTopicTests(TestCase):
    ...
    def test_board_topics_view_contains_link_back_to_homepage(self):
        board_topic_url = reverse('boards.topics', kwargs={'pk': 1})
...
class NewTopicTests(TestCase):
    ...
    def test_new_topic_invalud_post_data_empty_fields(self):
        url = reverse('boards.new_topic', kwargs={'pk': 1})
    ...
    def test_new_topic_url_resolves_board_topic_view(self):
        view = resolve('/board/1/new/')
        self.assertEquals(view.func, new_topic)

    def test_new_topic_view_contains_link_back_to_homepage(self):
        board_new_topic_url = reverse('boards.new_topic', kwargs={'pk': 1})
        response = self.client.get(board_new_topic_url)
        homepage_url = reverse('boards.home')
        self.assertContains(response, 'href="{0}"'.format(homepage_url))

    def test_new_topic_view_contains_link_back_to_topic(self):
        board_new_topic_url = reverse('boards.new_topic', kwargs={'pk': 1})
        response = self.client.get(board_new_topic_url)
        board_topic_url = reverse('boards.topics', kwargs={'pk': 1})
